...
class PointHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss(self, tb_dict=None):
        ...
        # print('2222222222', (point_cls_labels == 2).sum())
        # print('3333333333', (point_cls_labels == 3).sum())
        cls_weights[point_cls_labels == 2] *= 3
        cls_weights[point_cls_labels == 3] *= 6

        one_hot_targets = point_cls_preds.new_zeros(*list(point_cls_labels.shape), self.num_class + 1)
        one_hot_targets.scatter_(-1, (point_cls_labels * (point_cls_labels >= 0).long()).unsqueeze(dim=-1).long(), 1.0)
        one_hot_targets = one_hot_targets[..., 1:]
        cls_loss_src = self.cls_loss_func(point_cls_preds, one_hot_targets, weights=cls_weights)
        # if (point_cls_labels == 1).sum()>0:
        #     print('car: ', cls_loss_src[point_cls_labels == 1].mean())
        # else:
        #     print('car: ', 0)
        # if (point_cls_labels == 2).sum()>0:
        #     print('ped: ', cls_loss_src[point_cls_labels == 2].mean())
        # else:
        #     print('ped: ', 0)
        # if (point_cls_labels == 3).sum()>0:
        #     print('cyc: ', cls_loss_src[point_cls_labels == 3].mean())
        # else:
        #     print('cyc: ', 0)
