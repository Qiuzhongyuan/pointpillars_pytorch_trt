...
torch::Tensor
dense(torch::Tensor features, torch::Tensor indices, int batch_size, std::vector<int> spatialShape){
	...
	if(inputType == torch::kFloat32)
    {
        ...
        DenseSpace::cuda_scatter(features_rw, indices_rw, output_rw, spatialShape, num_voxels, num_features);
    }
    else if(inputType == torch::kHalf)
    {
	    ...
	    DenseSpace::cuda_scatter_fp16(features_rw, indices_rw, output_rw, spatialShape, num_voxels, num_features);
    }
    else
    {
...
torch::Tensor
dense_backward(torch::Tensor grad_output, torch::Tensor indices, std::vector<int> spatialShape){
    ...
    DenseSpace::cuda_scatter_backward(features_rw, indices_rw, output_rw, spatialShape, num_voxels, num_features);
