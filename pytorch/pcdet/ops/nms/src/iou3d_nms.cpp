...
nms(torch::Tensor batch_box, torch::Tensor batch_cls, int nms_pre_maxsize, int nms_post_maxsize, float nms_thresh, float score_thresh, int use_bev = 1){
    ...
    auto inputType = batch_box.scalar_type();
    int batch_size = batch_box.size(0);
    int num_box = batch_box.size(2);
    int num_cls = batch_cls.size(1);

    int num_out_info = use_bev == 0 ? 6 : 9;

    torch::Tensor index    = torch::zeros({num_box,}, torch::dtype(torch::kInt32).device(batch_box.device()));
    torch::Tensor score    = torch::zeros({num_box,}, torch::dtype(inputType).device(batch_box.device()));
    ...
    torch::Tensor box_temp = torch::zeros({num_out_info-2, nms_pre_maxsize}, torch::dtype(inputType).device(batch_box.device()));

    torch::Tensor pos      = torch::zeros({batch_size, 2}, torch::dtype(torch::kInt32).device(batch_box.device()));
    torch::Tensor ious     = torch::zeros({nms_pre_maxsize, nms_pre_maxsize}, torch::dtype(inputType).device(batch_box.device()));
    torch::Tensor outputs  = torch::zeros({batch_size, num_out_info, nms_post_maxsize}, torch::dtype(inputType).device(batch_box.device())) - 1;
    ...
    if(inputType == torch::kFloat32)
    {
        ...
        NMSSpace::cuda_nms(batch_box_rw, batch_cls_rw, score_rw, cls_type_rw, index_rw, pos_rw, cls_temp_index_rw, box_temp_index_rw,
                            ious_rw, outputs_rw, num_box, num_cls, nms_pre_maxsize, nms_post_maxsize, nms_thresh, batch_size, score_thresh, use_bev);
    }
