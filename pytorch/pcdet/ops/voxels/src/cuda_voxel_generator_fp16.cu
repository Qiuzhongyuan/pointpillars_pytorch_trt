...
void convertFP32ToFP16(float* in, __half* out, int num)
{
    ...
    checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ConvertFloat2Half));
...
void convertFP16ToFP32(__half* in, float* out, int num)
{
    ...
    checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ConvertHalf2Float));
...
void cuda_points_to_voxel_fp16(const __half* inPoints, const int* devNumValidPointsInput, int* outPosIndexBuff,
    ...
    for(int i = 0; i < batchSize; ++i)
    {
        ...
        checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, TraverseInputPointsPerBatchFP16, 0, numValidPointsInputPerBatch));
        ...
        *hCounterPerBatch = outvalidbatch;
        ...
        //Voxel
        checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, GetVoxelPerBatchFP16, 0, numValidPointsInputPerBatch));
        minGridSize = std::min(minGridSize, DivUp(numValidPointsInputPerBatch, blockSize));
        ...
        if(cluster_offset!=0)
        {
            checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ExtendVoxelClusterOffsetPerBatchFP16, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
        ...
        if(center_offset!=0)
        {
            checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ExtendVoxelCenterOffsetPerBatchFP16, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
        ...
        if(supplement!=0){
            //Voxel supplement
            checkCudaErrors(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, GetVoxelSupplementPerBatchFP16, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
