...
class WaymoDataset(DatasetTemplate):
    ...
    def drop_gt_out_of_range(self, gts):
        def IsValid(box):
            x, y, z = box[:3]
            if x> self.point_cloud_range[0] and y > self.point_cloud_range[1] and z > self.point_cloud_range[2] and \
                    x < self.point_cloud_range[3] and y < self.point_cloud_range[4] and z < self.point_cloud_range[5]:
                return True
            return False
        out = []
        for gt in gts:
            ...
            ret_info = {}
            keep_indices = []
            for i, box in enumerate(gt['annos']['gt_boxes_lidar']):
                if IsValid(box):
                    keep_indices.append(i)
            for key in gt['annos'].keys():
                ret_info[key] = gt['annos'][key][keep_indices]
            new_gt['annos'] = ret_info
            out.append(new_gt)
        return out
    ...
    def include_waymo_data(self, mode):
        ...
        waymo_infos = self.drop_gt_out_of_range(waymo_infos)
