...
size_t NMS3D::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs, int nbInputs, const nvinfer1::PluginTensorDesc* outputs, int nbOutputs) const
{
    ...
    size_t totalsize = 2*num_box*sizeof(float) + num_box*sizeof(int) + _nms_pre_maxsize*_nms_pre_maxsize*sizeof(float) + _nms_pre_maxsize*sizeof(float)*2 + sizeof(float)*_nms_pre_maxsize*(num_out_info-2)  + 
                        sizeof(float)*batch_size*num_box*(num_out_info-2) + sizeof(float)*batch_size*num_box*num_cls + sizeof(float)*batch_size*_nms_post_maxsize*num_out_info;
    return totalsize;
}
    ...
    cudaStream_t stream)
{
    ...
    void* cls_type_v = workspace + sizeof(int)*num_box*2;
    void* score_temp_v = workspace + sizeof(int)*num_box*3;
    void* cls_temp_v = workspace + sizeof(int)*num_box*3 + sizeof(float)*_nms_pre_maxsize;
    void* box_temp_v = workspace + sizeof(int)*num_box*3 + sizeof(float)*_nms_pre_maxsize*2;
    void* ious_v = workspace + sizeof(int)*num_box*3 + sizeof(float)*_nms_pre_maxsize*2 + sizeof(float)*_nms_pre_maxsize*(num_out_info-2);
    void* batch_box_v = workspace + 2*num_box*sizeof(float) + num_box*sizeof(int) + _nms_pre_maxsize*_nms_pre_maxsize*sizeof(float) + _nms_pre_maxsize*sizeof(float)*2 + sizeof(float)*_nms_pre_maxsize*(num_out_info-2);
    void* batch_cls_v = workspace + 2*num_box*sizeof(float) + num_box*sizeof(int) + _nms_pre_maxsize*_nms_pre_maxsize*sizeof(float) + _nms_pre_maxsize*sizeof(float)*2 + sizeof(float)*_nms_pre_maxsize*(num_out_info-2) + 
                        sizeof(float)*batch_size*num_box*(num_out_info-2);
    void* outfeature_v = workspace + 2*num_box*sizeof(float) + num_box*sizeof(int) + _nms_pre_maxsize*_nms_pre_maxsize*sizeof(float) + _nms_pre_maxsize*sizeof(float)*2 + sizeof(float)*_nms_pre_maxsize*(num_out_info-2) + 
                        sizeof(float)*batch_size*num_box*(num_out_info-2) + sizeof(float)*batch_size*num_box*num_cls;

    int* index = static_cast<int*>(index_v);
    float* score = static_cast<float*>(score_v);
    int* cls_type = static_cast<int*>(cls_type_v);
    float* score_temp = static_cast<float*>(score_temp_v);
    ...
    if (DataType::kHALF == inputDesc->type){
        ...
        half2float_nms(temp_box, batch_box, batch_size*num_box*(num_out_info-2));
        half2float_nms(temp_cls, batch_cls, batch_size*num_box*num_cls);
    }else{
    ...
    // LOG_ERROR(cudaMemset(index, 0, sizeof(int)*num_box));
    // LOG_ERROR(cudaMemset(score, 0, sizeof(float)*num_box));
    // LOG_ERROR(cudaMemset(cls_type, 0, sizeof(int)*num_box));
    // LOG_ERROR(cudaMemset(score_temp, 0, sizeof(float)*_nms_pre_maxsize));
    ...
    cudaError_t status = cuda_nms(stream, batch_box, batch_cls, score, cls_type, index, ious, outfeature, score_temp, cls_temp, box_temp, 
    num_box, num_cls, _nms_pre_maxsize, _nms_post_maxsize, _nms_thresh, batch_size, _score_thresh, _use_bev);

    if (DataType::kHALF == inputDesc->type){
        __half *outfeat = static_cast<__half *>(outputs[0]);
        float2half_nms(outfeature, outfeat, batch_size*_nms_post_maxsize*num_out_info);
    }else{
