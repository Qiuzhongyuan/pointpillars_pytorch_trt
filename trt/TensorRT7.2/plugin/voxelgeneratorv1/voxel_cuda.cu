...
void cuda_points_to_voxel(const float* inPoints, const int* devNumValidPointsInput, int* outPosIndexBuff,
    ...
    for(int i = 0; i < batchSize; ++i)
    {
        ...
        checkCudaErrors_voxelv1(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, TraverseInputPointsPerBatch, 0, numValidPointsInputPerBatch));
        ...
        *hCounterPerBatch = outvalidbatch;
        ...
        //Voxel
        checkCudaErrors_voxelv1(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, GetVoxelPerBatch, 0, numValidPointsInputPerBatch));
        minGridSize = std::min(minGridSize, DivUp(numValidPointsInputPerBatch, blockSize));
        ...
        if(cluster_offset!=0)
        {
            checkCudaErrors_voxelv1(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ExtendVoxelClusterOffsetPerBatch, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
        ...
        if(center_offset!=0)
        {
            checkCudaErrors_voxelv1(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, ExtendVoxelCenterOffsetPerBatch, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
        ...
        if(supplement!=0){
            //Voxel supplement
            checkCudaErrors_voxelv1(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, GetVoxelSupplementPerBatch, 0, *hCounterPerBatch));
            minGridSize = std::min(minGridSize, DivUp(*hCounterPerBatch, blockSize));
